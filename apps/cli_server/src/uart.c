...
int cmd_call_api(const char *str)
{
    if(str[0]>='a' && str[0]<='z')
    {
        bleuart_write("Enter m to show menu \n", 20);
        if(str[0] == 'm')
        {
            bleuart_write("r-start ranging \n", 17);
            bleuart_write("n-stop ranging \n", 16);
        }
        else
        {
        switch(*str)
        {
           case 'r': bleuart_write("RANGING STARTED \n", 18);
                     ranging_start();
                     break;
           case 'n': bleuart_write("RANGING STOPPED \n", 17);
                     ranging_stop();
                     break;
        }
        }
 ...
 */
int
ble_tag_uart_gatt_svr_init(void)
{
    int rc;
